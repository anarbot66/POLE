...
export default function DailyRewardsGrid({ currentUser }) {
  ...
  const [yearMonth,] = useState(new Date());
  ...
  const handleCollect = async day => {
    ...
    let apex = isWeekend(day) ? 0 : 500;
    let gs   = isWeekend(day) ? 50 : 0;
    if (weekendDates.has(key)) { apex = 500; gs = 50; }
              ...
              return (
                <div
                  ...
                  style={{
                    minHeight: 100,
                    padding: 8,
                    background: inMonth ? '#141416' : 'transparent',
                    color: !inMonth
                      ? '#555'
                      : (claimed || isPast)
                        ? '#404040'
